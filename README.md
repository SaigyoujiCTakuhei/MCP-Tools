...
| **协议** | streamable-http |
| **Python 版本** | >=3.10 |
| **总工具数** | 79 个（基础 34 + 番茄 6 + 网易云 13 + 进化 21 + PDF 2 + 思考引擎 3） |
...
```
E:\Codes\AI Related\MCP Server\v03\
...
│   ├── evolution/                  # 进化引擎模块
│   │   ├── __init__.py
│   │   ├── evolve_core.py          # 进化核心：流水线调度与状态管理（非暴露工具）
│   │   ├── evolution_state.json    # 进化状态持久化（JSON 增量索引）
│   │   ├── add_safety_pattern.py   # 自定义安全规则注册
│   │   ├── analyze_prompt.py       # Prompt 复杂度分析
│   │   ├── cancel_evolution.py     # 取消运行周期
│   │   ├── check_evolution_trigger.py # 检查触发条件
│   │   ├── check_safety.py         # 文本安全校验
│   │   ├── crossover_variants.py   # 变体交叉重组
│   │   ├── detect_anomalies.py     # 指标异常检测
│   │   ├── evaluate_variant.py     # 适应度评分
│   │   ├── explain_fitness.py      # 分数归因解析
│   │   ├── generate_ab_pair.py     # A/B 测试对生成
│   │   ├── generate_population.py  # 种群初始化
│   │   ├── get_evolution_status.py # 周期状态查询
│   │   ├── get_metrics_window.py   # 时间窗口聚合
│   │   ├── list_fitness_functions.py # 适配度函数列表
│   │   ├── mutate_prompt.py        # Prompt 变异
│   │   ├── record_metrics.py       # 任务指标记录
│   │   ├── register_fitness_function.py # 注册自定义函数
│   │   ├── resume_evolution.py     # 断点续跑
│   │   ├── start_evolution.py      # 启动新周期
│   │   ├── update_fitness_weights.py # 权重配置更新
│   │   └── validate_variant.py     # 变体全量安全验证
│   ├── fanqie/                     # 番茄小说工具
│   │   ├── __init__.py
...
│   ├── sequential_thinking/        # 深度思考引擎模块（新）
│   │   ├── __init__.py
│   │   ├── config.py               # 思考引擎：配置管理
│   │   ├── thinking_core.py        # 思考引擎：核心推理与状态机
│   │   └── tool_register.py        # 思考引擎：工具注册与桥接
│   └── tools/                      # 通用工具集
│       ├── __init__.py
│       ├── add.py                  # 基础数学
...
2. 硬编码导入核心工具模块
3. `_auto_scan_lib_tools()`：自动扫描 Lib 目录发现新工具（如新增的 PDF Reader、Fanqie、Netease、Sequential Thinking 模块）
...
### Lib/sequential_thinking — 深度思考引擎（新）

| 属性 | 值 |
|------|-----|
| **类型** | 逻辑推理模块 |
| **功能** | 复杂任务拆解、多步推理、状态机管理 |
| **核心文件** | `thinking_core.py`, `tool_register.py`, `config.py` |

**核心工具：**
- `thinking_core` 提供链式推理与上下文管理，`tool_register` 负责将推理结果桥接至 MCP 工具调用。

---

### Lib/evolution — 进化引擎核心

| 属性 | 值 |
|------|-----|
| **类型** | 自动化优化模块 |
| **功能** | Prompt 变体生成、适应度评估、安全沙箱、指标追踪 |
| **核心文件** | `evolve_core.py`（流水线调度）、`evolution_state.json`（状态持久化） |
| **暴露工具** | 21 个（详见下方工具集） |

**关键特性：**
- 全生命周期管理：生成 → 评估 → 变异 → 交叉 → 验证 → 持久化
- 增量索引机制：基于 JSON 结构追踪 Variant 状态，避免全量重载
- 安全隔离：内置注入检测与自定义规则热插拔
...
| 工具名 | 文件路径 | 功能描述 |
|--------|----------|----------|
| `start_evolution` | `Lib/evolution/start_evolution.py` | 启动进化周期（猫娘修改：触发后自动执行一代） |
| `get_evolution_status` | `Lib/evolution/get_evolution_status.py` | 获取进化周期状态 |
| `cancel_evolution` | `Lib/evolution/cancel_evolution.py` | 取消进化周期 |
| `resume_evolution` | `Lib/evolution/resume_evolution.py` | 从检查点恢复进化 |

### 🧪 变体生成（5 个）

| 工具名 | 文件路径 | 功能描述 |
|--------|----------|----------|
| `generate_population` | `Lib/evolution/generate_population.py` | 生成 Prompt 变体种群 |
| `mutate_prompt` | `Lib/evolution/mutate_prompt.py` | 变异单个 Prompt |
| `crossover_variants` | `Lib/evolution/crossover_variants.py` | 两个变体交叉生成后代 |
| `generate_ab_pair` | `Lib/evolution/generate_ab_pair.py` | 生成 A/B 测试对 |
| `analyze_prompt` | `Lib/evolution/analyze_prompt.py` | 分析 Prompt 复杂度 |

### 📊 适应度评估（5 个）

| 工具名 | 文件路径 | 功能描述 |
|--------|----------|----------|
| `evaluate_variant` | `Lib/evolution/evaluate_variant.py` | 评估变体适应度分数 |
| `explain_fitness` | `Lib/evolution/explain_fitness.py` | 解释适应度分解详情 |
| `register_fitness_function` | `Lib/evolution/register_fitness_function.py` | 注册自定义适应度函数 |
| `update_fitness_weights` | `Lib/evolution/update_fitness_weights.py` | 更新适应度权重 |
| `list_fitness_functions` | `Lib/evolution/list_fitness_functions.py` | 列出已注册的适应度函数 |

### 🛡 安全验证（3 个）

| 工具名 | 文件路径 | 功能描述 |
|--------|----------|----------|
| `validate_variant` | `Lib/evolution/validate_variant.py` | 验证变体安全性 |
| `check_safety` | `Lib/evolution/check_safety.py` | 检查文本安全性 |
| `add_safety_pattern` | `Lib/evolution/add_safety_pattern.py` | 添加自定义安全模式 |

### 📈 指标收集（4 个）

| 工具名 | 文件路径 | 功能描述 |
|--------|----------|----------|
| `record_metrics` | `Lib/evolution/record_metrics.py` | 记录任务指标数据 |
| `get_metrics_window` | `Lib/evolution/get_metrics_window.py` | 获取时间窗口聚合指标 |
| `check_evolution_trigger` | `Lib/evolution/check_evolution_trigger.py` | 检查是否触发进化 |
| `detect_anomalies` | `Lib/evolution/detect_anomalies.py` | 检测指标异常 |
...
```bash
# 进入项目目录
cd E:\Codes\AI Related\MCP Server\v03
...
Address: 127.0.0.1:58000
Tools: 79 (Success: 79, Failed: 0)
============================================================
```
...
Ent.py (入口)
  ├── 硬编码导入所有工具模块
  ├── _auto_scan_lib_tools() → 自动发现新工具（PDF/Fanqie/Netease/Sequential Thinking）
...
start_evolution() 触发
  └── evolve_core.py 调度
       ...
       └── Step 4: save → 持久化至 evolution_state.json
```
...
7. **根目录清理**：`????.txt` 为未知临时文件，建议确认无误后删除以避免干扰
8. **文件数量说明**：`Lib/evolution/` 目录下共 24 个文件（22 个 `.py` + 1 个 `.json` + 1 个 `__init__.py`），其中 21 个为暴露的 MCP 工具，`evolve_core.py` 为内部流水线调度核心，不参与工具注册
